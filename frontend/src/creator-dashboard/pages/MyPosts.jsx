...
export default function MyPosts() {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto animate-fade-in">
      <section aria-label="Page header" className="card bg-surface border border-default p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl md:text-2xl font-bold tracking-tight text-primary">My Posts</h1>
            <p className="text-secondary mt-1 text-xs md:text-sm font-medium">Manage all your content from one place.</p>
          </div>
      ...
      <section aria-label="My posts metrics" className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div onClick={() => handleSelectTab('Drafts')} className="stat-card bg-card border border-default cursor-pointer group">
          ...
          <div>
            <p className="text-[11px] font-semibold text-secondary uppercase tracking-wider">Draft Posts</p>
            <p className="text-2xl font-extrabold text-primary mt-1 tracking-tight">{stats.drafts}</p>
            <p className="text-[10px] text-secondary mt-0.5">In editing vault</p>
          </div>
        </div>

        <div onClick={() => handleSelectTab('Scheduled')} className="stat-card bg-card border border-default cursor-pointer group">
          ...
          <div>
            <p className="text-[11px] font-semibold text-secondary uppercase tracking-wider">Scheduled Posts</p>
            <p className="text-2xl font-extrabold text-primary mt-1 tracking-tight">{stats.scheduled}</p>
            <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold mt-0.5">▲ +2 active</p>
          </div>
        </div>

        <div onClick={() => handleSelectTab('Published')} className="stat-card bg-card border border-default cursor-pointer group">
          ...
          <div>
            <p className="text-[11px] font-semibold text-secondary uppercase tracking-wider">Published Posts</p>
            <p className="text-2xl font-extrabold text-primary mt-1 tracking-tight">{stats.published}</p>
            <p className="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold mt-0.5">▲ +12% this week</p>
          </div>
        </div>

        <div onClick={() => handleSelectTab('Rejected')} className="stat-card bg-card border border-default cursor-pointer group">
          ...
          <div>
            <p className="text-[11px] font-semibold text-secondary uppercase tracking-wider">Rejected Posts</p>
            <p className="text-2xl font-extrabold text-primary mt-1 tracking-tight">{stats.rejected}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 items-start">
        <div className="lg:col-span-3 space-y-6">
          <div className="card space-y-4 border border-default bg-card">
            <div className="flex items-center gap-1 border-b border-default pb-1 overflow-x-auto scrollbar-none">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="relative flex-1">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-4 py-2 text-xs bg-surface border border-default rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/30 text-primary"
                />
              </div>
              ...
              <div className="flex flex-wrap gap-2">
                <select 
                  ...
                  className="px-2.5 py-1.5 text-xs font-semibold bg-surface border border-default rounded-xl text-primary"
                >
          ...
          {filteredPosts.length === 0 ? (
            <div className="card flex flex-col items-center justify-center p-12 text-center border-dashed border-2 border-default bg-surface">
              <span className="text-4xl mb-3">📝</span>
              <h3 className="text-base font-bold text-primary">No Drafts Yet</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {filteredPosts.map((post) => {
                ...
                return (
                  <div key={post.id} className="card p-5 border border-default bg-card hover:border-indigo-500/40 transition-all duration-200 flex flex-col space-y-4">
                    <div className="flex justify-between items-start">
                      <div className="flex items-center gap-2.5">
                        <span className="text-2xl">{post.thumb}</span>
                        <div>
                          <h4 className="font-bold text-xs md:text-sm text-primary line-clamp-1">{post.title}</h4>
                          <span className="text-[10px] text-secondary font-medium">{post.edited}</span>
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-between border-t border-default pt-3 mt-auto">
                      <div className="flex items-center gap-1">
                        <button className="p-1.5 hover:bg-hover rounded-lg text-secondary transition-colors" title="Edit draft">
                          <Edit size={14} />
                        </button>
                        <button onClick={() => handleDuplicate(post)} className="p-1.5 hover:bg-hover rounded-lg text-secondary transition-colors" title="Duplicate">
                          <Copy size={14} />
                        </button>
            ...
            <div className="card p-0 overflow-hidden border border-default bg-card">
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead className="bg-surface border-b border-default">
                    <tr>
                      <th className="py-3 px-4 font-semibold text-secondary uppercase tracking-wider text-xs text-left">Post</th>
                      <th className="py-3 px-4 font-semibold text-secondary uppercase tracking-wider text-xs text-left">Platform</th>
                      <th className="py-3 px-4 font-semibold text-secondary uppercase tracking-wider text-xs text-left">Campaign</th>
                      <th className="py-3 px-4 font-semibold text-secondary uppercase tracking-wider text-xs text-left">Status</th>
                      <th className="py-3 px-4 font-semibold text-secondary uppercase tracking-wider text-xs text-left">Scheduled / Date</th>
                      <th className="py-3 px-4 font-semibold text-secondary uppercase tracking-wider text-xs text-right">Actions</th>
                    </tr>
                  </thead>
                        ...
                        <tr key={post.id} className="hover:bg-hover transition-colors">
                          <td className="py-3.5 px-4">
                            <div className="flex items-center gap-3">
                              <span className="w-8 h-8 rounded-lg bg-surface flex items-center justify-center text-lg">{post.thumb}</span>
                              <span className="font-bold text-primary line-clamp-1">{post.title}</span>
                            </div>
                          </td>
                          <td className="py-3.5 px-4">
                          ...
                          <td className="py-3.5 px-4 text-xs text-secondary font-medium">{post.campaign}</td>
                          <td className="py-3.5 px-4">
                            <StatusBadge status={post.status === 'Needs Review' ? 'review' : post.status} dot />
                          </td>
                          <td className="py-3.5 px-4 text-xs text-secondary font-medium">{post.time}</td>
                          <td className="py-3.5 px-4 text-right">
                            <div className="flex items-center justify-end gap-1.5">
                              <button className="p-1.5 hover:bg-hover rounded-lg text-secondary transition-colors" title="Preview">
                                <Eye size={14} />
                              </button>
                              <button className="p-1.5 hover:bg-hover rounded-lg text-secondary transition-colors" title="Edit">
                                <Edit size={14} />
                              </button>
                              <button onClick={() => handleDuplicate(post)} className="p-1.5 hover:bg-hover rounded-lg text-secondary transition-colors" title="Duplicate">
                                <Copy size={14} />
        ...
        <div className="space-y-6">
          <div className="card space-y-4 border border-default bg-card">
            <h2 className="text-xs md:text-sm font-bold text-primary border-b border-default pb-3 flex items-center gap-2">
            ...
            <div className="space-y-4 text-xs font-semibold text-secondary">
              ...
              <div className="flex justify-between border-b border-default pb-2">
                ...
                <span className="font-bold text-primary">{stats.scheduled} ready</span>
              </div>

              <div className="space-y-1.5">
                <div className="flex justify-between">
                  <span>Cloud Storage Used</span>
                  <span className="text-primary font-bold">4.2 GB / 10 GB</span>
                </div>
                <div className="h-1.5 bg-surface rounded-full overflow-hidden">
