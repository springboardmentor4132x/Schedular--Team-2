...
export default function MarketingReportsPage() {
  ...
  const handleGenerate = async (type) => {
    ...
    const title = type === 'monthly'
      ? `${new Date().toLocaleString('en-US', { month: 'long', year: 'numeric' })} — All Clients Report`
      : type === 'client'
    ...
    <div className="p-4 sm:p-6 max-w-[1400px] mx-auto">
      ...
      <PageHeader
        ...
        actions={
          <div className="flex gap-2">
            <button onClick={() => handleGenerate('monthly')} disabled={generating} className="flex items-center gap-1.5 px-3 h-9 rounded-[var(--r-md)] border text-xs font-semibold transition-all" style={{ background:'var(--card)', borderColor:'var(--border)', color:'var(--text)' }}>
              {generating ? <RefreshCw size={12} className="animate-spin" /> : <Calendar size={12} />} Monthly
            </button>
            <button onClick={() => handleGenerate('client')} disabled={generating || !activeClient} className="flex items-center gap-1.5 px-3 h-9 rounded-[var(--r-md)] border text-xs font-semibold transition-all" style={{ background:'var(--card)', borderColor:'var(--border)', color:'var(--text)' }}>
              <Users size={12} /> Client
            </button>
            <button onClick={() => handleGenerate('campaign')} disabled={generating} className="flex items-center gap-2 px-4 h-9 rounded-[var(--r-md)] text-sm font-semibold text-white hover:brightness-105" style={{ background:'linear-gradient(135deg,#1E3A8A,#4F46E5)' }}>
      ...
      <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.2 }} className="card overflow-hidden">
        ...
        <div className="px-4 py-3 border-t flex items-center justify-between" style={{ borderColor:'var(--border)', background:'var(--bg-alt)' }}>
          ...
          <button onClick={() => filtered.filter(r => r.status === 'ready').forEach(downloadCSV)} className="flex items-center gap-1.5 text-xs font-semibold hover:underline" style={{ color:'var(--primary)' }}>
