import { creatorReportDataMock, adminReportDataMock } from '../constants/reportMockData'

const delay = (ms = 150) => new Promise((resolve) => setTimeout(resolve, ms))

/**
 * Backend-Ready Report Service Layer
 */
export async function getReportData(reportType = 'engagement', filters = {}, role = 'creator') {
  await delay()
  const dataset = role === 'admin' ? adminReportDataMock : creatorReportDataMock
  const report = dataset[reportType] || dataset.engagement

  // Apply frontend mock filtering
  let filteredTable = [...report.tableData]
  if (filters.platform && filters.platform !== 'All') {
    filteredTable = filteredTable.filter(
      (item) => item.platform && item.platform.toLowerCase().includes(filters.platform.toLowerCase())
    )
  }
...
/**
 * Frontend Excel / CSV Export Engine
 */
export function exportReportExcel(reportType = 'engagement', filters = {}, data = {}, role = 'creator') {
  const roleTitle = role === 'admin' ? 'Admin' : 'Creator'
  const reportTitle = data.title || `${roleTitle} ${reportType} Report`
  const timestamp = new Date().toLocaleString()

  let csvContent = '\uFEFF' // UTF-8 BOM
  csvContent += `OrbitSocial - ${reportTitle}\n`
  csvContent += `Generated At,${timestamp}\n`
  csvContent += `Role,${roleTitle}\n`
  ...
  link.href = url
  link.setAttribute('download', `OrbitSocial_${role}_${reportType}_Report_${Date.now()}.csv`)
...
/**
 * Frontend PDF Print Export Engine
 */
export function exportReportPdf(reportType = 'engagement', filters = {}, data = {}, role = 'creator') {
  const roleTitle = role === 'admin' ? 'Admin' : 'Creator'
  const reportTitle = data.title || `${roleTitle} ${reportType} Report`
  const timestamp = new Date().toLocaleString()
    ...
    <html>
      ...
      <body>
        <div class="header">
          ...
          <div class="meta">Generated: ${timestamp} | Role: ${roleTitle}</div>
          <div class="filters">
        ...
        <div style="margin-top: 40px; font-size: 10px; color: #94a3b8; text-align: center; border-top: 1px solid #e2e8f0; padding-top: 10px;">
          OrbitSocial Platform Analytics &amp; Reports Module 8 • Confidential
        </div>
        <script>
