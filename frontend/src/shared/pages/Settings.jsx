...
function SettingsCard({ title, icon: Icon, children, id, className = '' }) {
  return (
    <Card id={id} className={`p-5 sm:p-7 flex flex-col gap-6 border border-default bg-card ${className}`}>
        ...
        <div className="space-y-6">
              ...
              <SettingsCard title="Account Info" icon={AccountIcon}>
                <div className="divide-y divide-slate-100 dark:divide-slate-700/60 text-sm">
                  <div className="py-3 flex justify-between gap-4 items-center">
                    ...
                    <span className="font-bold text-indigo-600 dark:text-indigo-400">{settings.accountInfo.currentPlan}</span>
                  </div>
                  <div className="py-3 flex justify-between gap-4 items-center">
            ...
            <SettingsCard title="Profile Summary" icon={AccountIcon}>
              <div className="flex flex-col items-center sm:items-start gap-5 p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-slate-100 dark:border-slate-700">
                <div className="flex flex-col sm:flex-row items-center gap-4 w-full">
                  <div className="w-16 h-16 rounded-full bg-gradient-to-br from-indigo-500 to-indigo-700 flex items-center justify-center text-white text-xl font-bold shadow-md flex-shrink-0">
                    JD
                  </div>
                  <div className="text-center sm:text-left min-w-0 flex-1">
                    ...
                    <div className="flex flex-wrap justify-center sm:justify-start gap-2 mt-2">
                      <span className="px-2.5 py-0.5 text-xs font-semibold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/40 rounded-full">
                        Role: Administrator
                      </span>
                      <span className="px-2.5 py-0.5 text-xs font-semibold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 rounded-full">
                        Status: Active
                      </span>
                    </div>
                  </div>
          ...
          {activeTab === 'notifications' && (
            <SettingsCard title="Notification Toggles" icon={BellIcon}>
              <div className="space-y-5">
                {[
                  { key: 'emailNotifications', label: 'Email Notifications', desc: 'Receive real-time system alerts, digests, and reports via email.' },
                  { key: 'pushNotifications', label: 'Push Notifications', desc: 'Enable push alerts in your browser window.' },
                  { key: 'publishingNotifications', label: 'Publishing Status Notifications', desc: 'Alerts when posts publish successfully or encounter errors.' },
                  { key: 'scheduledReminders', label: 'Scheduled Post Reminders', desc: 'Reminders before posts publish according to schedule.' },
                  { key: 'campaignAlerts', label: 'Campaign Alerts', desc: 'Notifications about brand campaign deadlines, briefs, and ROI.' },
                  { key: 'accountActivity', label: 'Account Activity Updates', desc: 'Immediate warnings for new sign-ins, security, and account updates.' },
                  { key: 'teamCollaboration', label: 'Team Collaboration Alerts', desc: 'Alerts when team members assign drafts, comment, or update content.' },
                ].map((item) => (
                  <div key={item.key} className="flex items-start justify-between gap-4 p-3 hover:bg-slate-50 dark:hover:bg-slate-800/40 rounded-xl transition-colors duration-150">
                    <div className="flex flex-col">
                      <label htmlFor={`toggle-notif-${item.key}`} className="text-sm font-semibold text-slate-800 dark:text-slate-200 cursor-pointer">
                        {item.label}
                      </label>
                    ...
                    <button
                      ...
                      className={[
                        ...
                        selectedTheme === option.value
                          ? 'bg-indigo-50 border-indigo-600 text-indigo-700 dark:bg-indigo-950/40 dark:border-indigo-400 dark:text-indigo-300 font-bold'
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={platform.name} className="p-4 bg-slate-50 dark:bg-slate-800/40 border border-slate-100 dark:border-slate-700 rounded-2xl flex items-center gap-3">
                      ...
                      <span className="px-2 py-0.5 text-[10px] font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 rounded-full">
                        {platform.status}
                      </span>
                    </div>
                  ))}
          ...
          {activeTab === 'billing' && (
            <SettingsCard title="Subscription & Billing" icon={BillingIcon}>
              <div className="space-y-5">
                <div className="p-4 bg-indigo-50/40 dark:bg-indigo-950/15 border border-indigo-100/60 dark:border-indigo-900/40 rounded-2xl">
                  <h4 className="text-sm font-bold text-indigo-800 dark:text-indigo-400">Current Subscription</h4>
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-3.5">
                  <div className="space-y-1.5">
                    <label className="label-base" htmlFor="support-subject">Subject</label>
                    <input
                      type="text"
                      id="support-subject"
                  ...
                  <div className="space-y-1.5">
                    <label className="label-base" htmlFor="support-message">Message</label>
                    <textarea
                      id="support-message"
                      rows="4"
                  ...
                  <div className="flex justify-end gap-3 pt-2">
                    <Button variant="outline" size="md" onClick={() => navigate('/dashboard')}>Cancel</Button>
