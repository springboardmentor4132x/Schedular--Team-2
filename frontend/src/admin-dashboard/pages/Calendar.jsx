  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <Card className="p-12 flex flex-col items-center justify-center text-center min-h-[340px]">
        <CalendarLargeIcon />
        <h3 className="text-lg font-bold text-primary mt-5">Content Calendar</h3>
      ...
      <Card className="p-6">
        <h3 className="text-base font-bold text-primary mb-4">Upcoming This Week</h3>
          ...
          {[
            ...
            <div key={i} className="flex items-center gap-4 p-3 rounded-xl hover:bg-hover transition-colors">
              <div className="w-14 text-center flex-shrink-0">
                <p className="text-xs font-semibold text-secondary uppercase">{item.day}</p>
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-primary truncate">{item.text}</p>
                <p className="text-xs text-secondary mt-0.5">{item.platform}</p>
              </div>
              <span className={`text-xs px-2.5 py-1 rounded-full font-medium whitespace-nowrap ${
                item.status === 'Scheduled' ? 'bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-400' :
                item.status === 'Draft' ? 'bg-surface text-secondary border border-default' :
