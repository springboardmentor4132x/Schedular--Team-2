import React, { useState, useEffect } from 'react'
import {
  Users,
  Building2,
  Target,
  CheckCircle2,
  XCircle,
  Clock,
  Search,
  Filter,
  Download,
  Plus,
  MoreVertical,
  Eye,
  Edit2,
  UserPlus,
  Power,
  AlertOctagon,
  Trash2,
  ArrowUpDown,
  RotateCcw,
  Sparkles
} from 'lucide-react'

import Card from '../../shared/components/ui/Card'
import Button from '../../shared/components/Button'
import Avatar from '../../shared/components/ui/Avatar'
import StatusBadge from '../../shared/components/ui/StatusBadge'

import {
  getMarketingTeams,
  createMarketingTeam,
  updateMarketingTeam,
  updateMarketingTeamStatus,
  addTeamMember,
  removeTeamMember,
  assignBusinessAccount,
  removeBusinessAccount,
  deleteMarketingTeam,
  exportMarketingTeamsCsv
} from '../services/marketingTeamsService'

import TeamDetailsModal from '../components/teams/TeamDetailsModal'
import TeamFormModal from '../components/teams/TeamFormModal'
import ManageMembersModal from '../components/teams/ManageMembersModal'
import ManageAccountsModal from '../components/teams/ManageAccountsModal'
import TeamConfirmModal from '../components/teams/TeamConfirmModal'

export default function MarketingTeamsManagement() {
  const [teamsList, setTeamsList] = useState([])
  const [loading, setLoading] = useState(true)

  // Filter States
  const [searchQuery, setSearchQuery] = useState('')
  const [statusFilter, setStatusFilter] = useState('All')
  const [typeFilter, setTypeFilter] = useState('All')
  const [campaignStatusFilter, setCampaignStatusFilter] = useState('All')

  // Sort State
  const [sortField, setSortField] = useState('name')
  const [sortOrder, setSortOrder] = useState('asc')

  // Pagination State
  const [currentPage, setCurrentPage] = useState(1)
  const pageSize = 8

  // Modals & Active Menu State
  const [selectedTeam, setSelectedTeam] = useState(null)
  const [isViewModalOpen, setIsViewModalOpen] = useState(false)
  const [isFormModalOpen, setIsFormModalOpen] = useState(false)
  const [isMembersModalOpen, setIsMembersModalOpen] = useState(false)
  const [isAccountsModalOpen, setIsAccountsModalOpen] = useState(false)
  const [editingTeam, setEditingTeam] = useState(null)
  const [activeMenuId, setActiveMenuId] = useState(null)

  // Confirm Modal State
  const [confirmModal, setConfirmModal] = useState({
    isOpen: false,
    title: '',
    message: '',
    confirmVariant: 'danger',
    confirmText: 'Confirm',
    onConfirm: () => {}
  })

  const loadData = async () => {
    setLoading(true)
    const data = await getMarketingTeams()
    setTeamsList(data)
    setLoading(false)
  }

  useEffect(() => {
    loadData()
  }, [])

  // KPI Calculations
  const totalTeamsCount = teamsList.length
  const activeTeamsCount = teamsList.filter((t) => t.status === 'Active').length
  const totalMembersCount = teamsList.reduce((acc, curr) => acc + (curr.members ? curr.members.length : 0), 0)
  const totalActiveCampaignsCount = teamsList.reduce((acc, curr) => acc + (curr.activeCampaignsCount || 0), 0)
  const totalAccountsManagedCount = teamsList.reduce(
    (acc, curr) => acc + (curr.businessAccounts ? curr.businessAccounts.length : 0),
    0
  )

  // Filter Logic
  const filteredTeams = teamsList.filter((team) => {
    const matchesSearch =
      searchQuery.trim() === '' ||
      team.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
      (team.lead && team.lead.name.toLowerCase().includes(searchQuery.toLowerCase()))

    const matchesStatus = statusFilter === 'All' || team.status === statusFilter
    const matchesType = typeFilter === 'All' || team.type === typeFilter

    let matchesCampaignStatus = true
    if (campaignStatusFilter === 'Active') {
      matchesCampaignStatus = (team.activeCampaignsCount || 0) > 0
    } else if (campaignStatusFilter === 'NoActive') {
      matchesCampaignStatus = (team.activeCampaignsCount || 0) === 0
    }

    return matchesSearch && matchesStatus && matchesType && matchesCampaignStatus
  })

  // Sort Logic
  const sortedTeams = [...filteredTeams].sort((a, b) => {
    let aVal = a[sortField] || ''
    let bVal = b[sortField] || ''

    if (sortField === 'name') {
      aVal = a.name.toLowerCase()
      bVal = b.name.toLowerCase()
    } else if (sortField === 'members') {
      aVal = a.members ? a.members.length : 0
      bVal = b.members ? b.members.length : 0
    } else if (sortField === 'activeCampaignsCount') {
      aVal = a.activeCampaignsCount || 0
      bVal = b.activeCampaignsCount || 0
    } else if (sortField === 'createdDate') {
      aVal = a.createdDate
      bVal = b.createdDate
    }

    if (aVal < bVal) return sortOrder === 'asc' ? -1 : 1
    if (aVal > bVal) return sortOrder === 'asc' ? 1 : -1
    return 0
  })

  // Pagination Logic
  const totalPages = Math.ceil(sortedTeams.length / pageSize) || 1
  const paginatedTeams = sortedTeams.slice((currentPage - 1) * pageSize, currentPage * pageSize)

  const handleSortToggle = (field) => {
    if (sortField === field) {
      setSortOrder(sortOrder === 'asc' ? 'desc' : 'asc')
    } else {
      setSortField(field)
      setSortOrder('asc')
    }
  }

  const handleClearFilters = () => {
    setSearchQuery('')
    setStatusFilter('All')
    setTypeFilter('All')
    setCampaignStatusFilter('All')
    setCurrentPage(1)
  }

  // Handlers
  const handleOpenCreateModal = () => {
    setEditingTeam(null)
    setIsFormModalOpen(true)
  }

  const handleOpenEditModal = (team) => {
    setEditingTeam(team)
    setIsFormModalOpen(true)
    setActiveMenuId(null)
  }

  const handleOpenViewModal = (team) => {
    setSelectedTeam(team)
    setIsViewModalOpen(true)
    setActiveMenuId(null)
  }

  const handleOpenMembersModal = (team) => {
    setSelectedTeam(team)
    setIsMembersModalOpen(true)
    setActiveMenuId(null)
  }

  const handleOpenAccountsModal = (team) => {
    setSelectedTeam(team)
    setIsAccountsModalOpen(true)
    setActiveMenuId(null)
  }

  const handleSaveTeam = async (formData) => {
    if (editingTeam) {
      const updated = await updateMarketingTeam(editingTeam.id, formData)
      setTeamsList((prev) => prev.map((t) => (t.id === editingTeam.id ? updated : t)))
    } else {
      const created = await createMarketingTeam(formData)
      setTeamsList((prev) => [created, ...prev])
    }
  }

  const handleAddMember = async (teamId, memberData) => {
    const updated = await addTeamMember(teamId, memberData)
    setTeamsList((prev) => prev.map((t) => (t.id === teamId ? updated : t)))
    if (selectedTeam && selectedTeam.id === teamId) setSelectedTeam(updated)
  }

  const handleRemoveMember = async (teamId, memberId) => {
    const updated = await removeTeamMember(teamId, memberId)
    setTeamsList((prev) => prev.map((t) => (t.id === teamId ? updated : t)))
    if (selectedTeam && selectedTeam.id === teamId) setSelectedTeam(updated)
  }

  const handleAssignAccount = async (teamId, accountData) => {
    const updated = await assignBusinessAccount(teamId, accountData)
    setTeamsList((prev) => prev.map((t) => (t.id === teamId ? updated : t)))
    if (selectedTeam && selectedTeam.id === teamId) setSelectedTeam(updated)
  }

  const handleRemoveAccount = async (teamId, accountId) => {
    const updated = await removeBusinessAccount(teamId, accountId)
    setTeamsList((prev) => prev.map((t) => (t.id === teamId ? updated : t)))
    if (selectedTeam && selectedTeam.id === teamId) setSelectedTeam(updated)
  }

  const handleToggleStatusConfirm = (team) => {
    setActiveMenuId(null)
    const nextStatus = team.status === 'Active' ? 'Inactive' : 'Active'
    setConfirmModal({
      isOpen: true,
      title: `${nextStatus === 'Active' ? 'Activate' : 'Deactivate'} Marketing Team`,
      message: `Are you sure you want to change status for "${team.name}" to "${nextStatus}"?`,
      confirmVariant: nextStatus === 'Active' ? 'primary' : 'warning',
      confirmText: nextStatus === 'Active' ? 'Activate Team' : 'Deactivate Team',
      onConfirm: async () => {
        const updated = await updateMarketingTeamStatus(team.id, nextStatus)
        setTeamsList((prev) => prev.map((t) => (t.id === team.id ? updated : t)))
      }
    })
  }

  const handleSuspendConfirm = (team) => {
    setActiveMenuId(null)
    setConfirmModal({
      isOpen: true,
      title: `Suspend Marketing Team`,
      message: `Are you sure you want to suspend "${team.name}"? Suspended teams cannot assign members or manage campaigns.`,
      confirmVariant: 'danger',
      confirmText: 'Suspend Team',
      onConfirm: async () => {
        const updated = await updateMarketingTeamStatus(team.id, 'Suspended')
        setTeamsList((prev) => prev.map((t) => (t.id === team.id ? updated : t)))
      }
    })
  }

  const handleDeleteConfirm = (team) => {
    setActiveMenuId(null)
    setConfirmModal({
      isOpen: true,
      title: `Delete Marketing Team`,
      message: `Are you sure you want to permanently delete marketing team "${team.name}"? This action cannot be undone.`,
      confirmVariant: 'danger',
      confirmText: 'Delete Permanently',
      onConfirm: async () => {
        await deleteMarketingTeam(team.id)
        setTeamsList((prev) => prev.filter((t) => t.id !== team.id))
      }
    })
  }

  const handleExportCsv = () => {
    exportMarketingTeamsCsv(filteredTeams)
  }

  const isFiltersActive = searchQuery || statusFilter !== 'All' || typeFilter !== 'All' || campaignStatusFilter !== 'All'

  return (
    <div className="max-w-7xl mx-auto space-y-6 animate-fade-in pb-16">
      {/* 1. PAGE HEADER */}
      <section aria-label="Marketing Teams Header" className="card relative overflow-hidden bg-gradient-to-r from-indigo-50/50 via-white to-purple-50/40 dark:from-indigo-950/30 dark:via-slate-900 dark:to-slate-900 border border-slate-200/80 dark:border-slate-800 p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-xl md:text-2xl font-bold tracking-tight text-slate-900 dark:text-slate-50">
              Marketing Teams
            </h1>
            <p className="text-slate-500 dark:text-slate-400 mt-1 text-xs md:text-sm font-medium">
              Manage teams, members, business accounts, and campaign responsibilities.
            </p>
          </div>

          <div className="flex items-center gap-2.5 shrink-0">
            <Button
              variant="outline"
              size="sm"
              onClick={handleExportCsv}
              className="flex items-center gap-1.5 font-bold text-xs"
            >
              <Download size={14} />
              <span>Export</span>
            </Button>

            <Button
              variant="primary"
              size="sm"
              onClick={handleOpenCreateModal}
              className="flex items-center gap-1.5 font-bold text-xs"
            >
              <Plus size={14} />
              <span>+ Create Team</span>
            </Button>
          </div>
        </div>
      </section>

      {/* 2. KPI SUMMARY CARDS */}
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3.5">
        <Card className="p-4 border border-slate-200/80 dark:border-slate-800 flex items-center justify-between">
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Total Teams</span>
            <span className="text-xl font-extrabold text-slate-900 dark:text-slate-100 mt-0.5 block">{totalTeamsCount}</span>
          </div>
          <div className="w-10 h-10 rounded-xl bg-indigo-50 dark:bg-indigo-950/60 text-indigo-600 dark:text-indigo-400 flex items-center justify-center">
            <Users size={18} />
          </div>
        </Card>

        <Card className="p-4 border border-slate-200/80 dark:border-slate-800 flex items-center justify-between">
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Active Teams</span>
            <span className="text-xl font-extrabold text-emerald-600 dark:text-emerald-400 mt-0.5 block">{activeTeamsCount}</span>
          </div>
          <div className="w-10 h-10 rounded-xl bg-emerald-50 dark:bg-emerald-950/60 text-emerald-600 dark:text-emerald-400 flex items-center justify-center">
            <CheckCircle2 size={18} />
          </div>
        </Card>

        <Card className="p-4 border border-slate-200/80 dark:border-slate-800 flex items-center justify-between">
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Total Members</span>
            <span className="text-xl font-extrabold text-purple-600 dark:text-purple-400 mt-0.5 block">{totalMembersCount}</span>
          </div>
          <div className="w-10 h-10 rounded-xl bg-purple-50 dark:bg-purple-950/60 text-purple-600 dark:text-purple-400 flex items-center justify-center">
            <UserPlus size={18} />
          </div>
        </Card>

        <Card className="p-4 border border-slate-200/80 dark:border-slate-800 flex items-center justify-between">
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Active Campaigns</span>
            <span className="text-xl font-extrabold text-sky-600 dark:text-sky-400 mt-0.5 block">{totalActiveCampaignsCount}</span>
          </div>
          <div className="w-10 h-10 rounded-xl bg-sky-50 dark:bg-sky-950/60 text-sky-600 dark:text-sky-400 flex items-center justify-center">
            <Target size={18} />
          </div>
        </Card>

        <Card className="p-4 border border-slate-200/80 dark:border-slate-800 flex items-center justify-between col-span-2 sm:col-span-1">
          <div>
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Accounts Managed</span>
            <span className="text-xl font-extrabold text-amber-600 dark:text-amber-400 mt-0.5 block">{totalAccountsManagedCount}</span>
          </div>
          <div className="w-10 h-10 rounded-xl bg-amber-50 dark:bg-amber-950/60 text-amber-600 dark:text-amber-400 flex items-center justify-center">
            <Building2 size={18} />
          </div>
        </Card>
      </div>

      {/* 3. SEARCH + FILTER TOOLBAR */}
      <Card className="p-4 border border-slate-200/80 dark:border-slate-800 space-y-3">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 items-center">
          {/* Search Input */}
          <div className="relative lg:col-span-2">
            <Search size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
            <input
              type="text"
              placeholder="Search teams by name or team lead..."
              value={searchQuery}
              onChange={(e) => {
                setSearchQuery(e.target.value)
                setCurrentPage(1)
              }}
              className="w-full pl-9 pr-4 py-1.5 text-xs rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none"
            />
          </div>

          {/* Status Filter */}
          <div className="space-y-1">
            <select
              value={statusFilter}
              onChange={(e) => {
                setStatusFilter(e.target.value)
                setCurrentPage(1)
              }}
              className="w-full input-base select-base text-xs py-1.5"
            >
              <option value="All">All Status</option>
              <option value="Active">Active</option>
              <option value="Inactive">Inactive</option>
              <option value="Suspended">Suspended</option>
            </select>
          </div>

          {/* Team Type Filter */}
          <div className="space-y-1">
            <select
              value={typeFilter}
              onChange={(e) => {
                setTypeFilter(e.target.value)
                setCurrentPage(1)
              }}
              className="w-full input-base select-base text-xs py-1.5"
            >
              <option value="All">All Team Types</option>
              <option value="Internal">Internal</option>
              <option value="Agency">Agency</option>
              <option value="Brand">Brand</option>
              <option value="Hybrid">Hybrid</option>
            </select>
          </div>

          {/* Campaign Status Filter */}
          <div className="space-y-1">
            <select
              value={campaignStatusFilter}
              onChange={(e) => {
                setCampaignStatusFilter(e.target.value)
                setCurrentPage(1)
              }}
              className="w-full input-base select-base text-xs py-1.5"
            >
              <option value="All">All Campaigns</option>
              <option value="Active">Has Active Campaigns</option>
              <option value="NoActive">No Active Campaigns</option>
            </select>
          </div>
        </div>

        {isFiltersActive && (
          <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800">
            <span className="text-xs text-slate-500 font-medium">
              Active filters applied ({filteredTeams.length} teams found)
            </span>
            <button
              onClick={handleClearFilters}
              className="text-xs font-bold text-indigo-600 dark:text-indigo-400 hover:underline flex items-center gap-1 cursor-pointer"
            >
              <RotateCcw size={12} />
              <span>Clear Filters</span>
            </button>
          </div>
        )}
      </Card>

      {/* 4. TEAM TABLE */}
      <Card className="p-0 border border-slate-200/80 dark:border-slate-800 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse text-xs">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-800/80 border-b border-slate-200/80 dark:border-slate-800 text-slate-700 dark:text-slate-300 font-bold uppercase tracking-wider text-[11px]">
                <th
                  onClick={() => handleSortToggle('name')}
                  className="p-4 whitespace-nowrap cursor-pointer hover:text-indigo-600 transition-colors"
                >
                  <div className="flex items-center gap-1.5">
                    <span>TEAM</span>
                    <ArrowUpDown size={12} className="text-slate-400" />
                  </div>
                </th>
                <th className="p-4 whitespace-nowrap">TEAM LEAD</th>
                <th
                  onClick={() => handleSortToggle('members')}
                  className="p-4 whitespace-nowrap cursor-pointer hover:text-indigo-600 transition-colors"
                >
                  <div className="flex items-center gap-1.5">
                    <span>MEMBERS</span>
                    <ArrowUpDown size={12} className="text-slate-400" />
                  </div>
                </th>
                <th className="p-4 whitespace-nowrap">BUSINESS ACCOUNTS</th>
                <th
                  onClick={() => handleSortToggle('activeCampaignsCount')}
                  className="p-4 whitespace-nowrap cursor-pointer hover:text-indigo-600 transition-colors"
                >
                  <div className="flex items-center gap-1.5">
                    <span>ACTIVE CAMPAIGNS</span>
                    <ArrowUpDown size={12} className="text-slate-400" />
                  </div>
                </th>
                <th className="p-4 whitespace-nowrap">STATUS</th>
                <th
                  onClick={() => handleSortToggle('createdDate')}
                  className="p-4 whitespace-nowrap cursor-pointer hover:text-indigo-600 transition-colors"
                >
                  <div className="flex items-center gap-1.5">
                    <span>CREATED</span>
                    <ArrowUpDown size={12} className="text-slate-400" />
                  </div>
                </th>
                <th className="p-4 whitespace-nowrap text-right">ACTIONS</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60 text-slate-800 dark:text-slate-200">
              {loading ? (
                <tr>
                  <td colSpan={8} className="p-12 text-center text-xs text-slate-400">
                    Loading marketing teams...
                  </td>
                </tr>
              ) : paginatedTeams.length === 0 ? (
                <tr>
                  <td colSpan={8} className="p-12 text-center text-xs text-slate-400">
                    No marketing teams found matching your search or filters.
                  </td>
                </tr>
              ) : (
                paginatedTeams.map((team) => (
                  <tr
                    key={team.id}
                    className="hover:bg-slate-50/80 dark:hover:bg-slate-800/40 transition-colors duration-150 relative"
                  >
                    {/* Team Name & Description */}
                    <td className="p-4 whitespace-nowrap">
                      <div className="flex items-center gap-3">
                        <Avatar src={team.avatar} alt={team.name} size="sm" />
                        <div>
                          <p className="font-bold text-slate-900 dark:text-slate-100 text-xs">
                            {team.name}
                          </p>
                          <span className="text-[10px] font-bold text-indigo-600 dark:text-indigo-400">
                            {team.type} Team
                          </span>
                        </div>
                      </div>
                    </td>

                    {/* Team Lead */}
                    <td className="p-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        <Avatar src={team.lead?.avatar} alt={team.lead?.name || 'Lead'} size="xs" />
                        <span className="font-semibold text-slate-800 dark:text-slate-200">
                          {team.lead?.name || 'Unassigned'}
                        </span>
                      </div>
                    </td>

                    {/* Members Avatar Stack & Count */}
                    <td className="p-4 whitespace-nowrap">
                      <div className="flex items-center gap-2">
                        <div className="flex -space-x-2 overflow-hidden">
                          {(team.members || []).slice(0, 3).map((m, idx) => (
                            <img
                              key={m.id || idx}
                              className="inline-block h-6 w-6 rounded-full ring-2 ring-white dark:ring-slate-900 object-cover"
                              src={m.avatar}
                              alt={m.name}
                            />
                          ))}
                        </div>
                        <span className="font-extrabold text-slate-900 dark:text-slate-100">
                          {team.members?.length || 0}
                        </span>
                      </div>
                    </td>

                    {/* Business Accounts Count */}
                    <td className="p-4 whitespace-nowrap font-extrabold text-slate-900 dark:text-slate-100">
                      {team.businessAccounts?.length || 0} Accounts
                    </td>

                    {/* Active Campaigns Count */}
                    <td className="p-4 whitespace-nowrap font-extrabold text-indigo-600 dark:text-indigo-400">
                      {team.activeCampaignsCount || 0} Active
                    </td>

                    {/* Status Badge */}
                    <td className="p-4 whitespace-nowrap">
                      <StatusBadge status={team.status} dot />
                    </td>

                    {/* Created Date */}
                    <td className="p-4 whitespace-nowrap font-medium text-slate-500">
                      {team.createdDate}
                    </td>

                    {/* Actions Menu */}
                    <td className="p-4 whitespace-nowrap text-right relative">
                      <div className="inline-block text-left">
                        <button
                          type="button"
                          onClick={() => setActiveMenuId(activeMenuId === team.id ? null : team.id)}
                          className="p-1.5 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 dark:text-slate-400 transition-colors cursor-pointer"
                        >
                          <MoreVertical size={16} />
                        </button>

                        {/* Dropdown Menu */}
                        {activeMenuId === team.id && (
                          <div className="absolute right-4 mt-1 w-48 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-slate-200/80 dark:border-slate-700 z-50 py-1 text-left animate-scale-in">
                            <button
                              onClick={() => handleOpenViewModal(team)}
                              className="w-full px-3 py-2 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/60 flex items-center gap-2"
                            >
                              <Eye size={14} className="text-indigo-500" />
                              <span>View Team</span>
                            </button>

                            <button
                              onClick={() => handleOpenEditModal(team)}
                              className="w-full px-3 py-2 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/60 flex items-center gap-2"
                            >
                              <Edit2 size={14} className="text-blue-500" />
                              <span>Edit Team</span>
                            </button>

                            <button
                              onClick={() => handleOpenMembersModal(team)}
                              className="w-full px-3 py-2 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/60 flex items-center gap-2"
                            >
                              <UserPlus size={14} className="text-purple-500" />
                              <span>Manage Members</span>
                            </button>

                            <button
                              onClick={() => handleOpenAccountsModal(team)}
                              className="w-full px-3 py-2 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/60 flex items-center gap-2"
                            >
                              <Building2 size={14} className="text-amber-500" />
                              <span>Manage Accounts</span>
                            </button>

                            <button
                              onClick={() => handleToggleStatusConfirm(team)}
                              className="w-full px-3 py-2 text-xs font-semibold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700/60 flex items-center gap-2"
                            >
                              <Power size={14} className="text-emerald-500" />
                              <span>{team.status === 'Active' ? 'Deactivate' : 'Activate'}</span>
                            </button>

                            <button
                              onClick={() => handleSuspendConfirm(team)}
                              className="w-full px-3 py-2 text-xs font-semibold text-amber-600 dark:text-amber-400 hover:bg-amber-50 dark:hover:bg-amber-950/40 flex items-center gap-2"
                            >
                              <AlertOctagon size={14} />
                              <span>Suspend Team</span>
                            </button>

                            <div className="border-t border-slate-100 dark:border-slate-700 my-1" />

                            <button
                              onClick={() => handleDeleteConfirm(team)}
                              className="w-full px-3 py-2 text-xs font-semibold text-rose-600 dark:text-rose-400 hover:bg-rose-50 dark:hover:bg-rose-950/40 flex items-center gap-2"
                            >
                              <Trash2 size={14} />
                              <span>Delete Team</span>
                            </button>
                          </div>
                        )}
                      </div>
                    </td>
                  </tr>
                ))
              )}
            </tbody>
          </table>
        </div>

        {/* 5. PAGINATION */}
        {sortedTeams.length > 0 && (
          <div className="p-4 bg-slate-50/60 dark:bg-slate-800/40 border-t border-slate-100 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
            <span className="text-slate-500 font-medium">
              Showing {(currentPage - 1) * pageSize + 1}–{Math.min(currentPage * pageSize, sortedTeams.length)} of {sortedTeams.length} teams
            </span>

            <div className="flex items-center gap-1.5">
              <Button
                variant="outline"
                size="xs"
                disabled={currentPage === 1}
                onClick={() => setCurrentPage((prev) => Math.max(prev - 1, 1))}
              >
                Previous
              </Button>

              {Array.from({ length: totalPages }, (_, i) => i + 1).map((pageNum) => (
                <button
                  key={pageNum}
                  onClick={() => setCurrentPage(pageNum)}
                  className={`w-7 h-7 rounded-lg text-xs font-bold transition-all ${
                    currentPage === pageNum
                      ? 'bg-indigo-600 text-white'
                      : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-400 border border-slate-200/80 dark:border-slate-700 hover:bg-slate-100'
                  }`}
                >
                  {pageNum}
                </button>
              ))}

              <Button
                variant="outline"
                size="xs"
                disabled={currentPage === totalPages}
                onClick={() => setCurrentPage((prev) => Math.min(prev + 1, totalPages))}
              >
                Next
              </Button>
            </div>
          </div>
        )}
      </Card>

      {/* MODALS INTEGRATION */}
      <TeamDetailsModal
        isOpen={isViewModalOpen}
        onClose={() => setIsViewModalOpen(false)}
        team={selectedTeam}
      />

      <TeamFormModal
        isOpen={isFormModalOpen}
        onClose={() => setIsFormModalOpen(false)}
        onSave={handleSaveTeam}
        team={editingTeam}
      />

      <ManageMembersModal
        isOpen={isMembersModalOpen}
        onClose={() => setIsMembersModalOpen(false)}
        onAddMember={handleAddMember}
        onRemoveMember={handleRemoveMember}
        team={selectedTeam}
      />

      <ManageAccountsModal
        isOpen={isAccountsModalOpen}
        onClose={() => setIsAccountsModalOpen(false)}
        onAssignAccount={handleAssignAccount}
        onRemoveAccount={handleRemoveAccount}
        team={selectedTeam}
      />

      <TeamConfirmModal
        isOpen={confirmModal.isOpen}
        onClose={() => setConfirmModal((prev) => ({ ...prev, isOpen: false }))}
        onConfirm={confirmModal.onConfirm}
        title={confirmModal.title}
        message={confirmModal.message}
        confirmVariant={confirmModal.confirmVariant}
        confirmText={confirmModal.confirmText}
      />
    </div>
  )
}
