...
export default function ManagePlatformsModal({ isOpen, onClose, onSave, account }) {
  const allPlatforms = ['Instagram', 'Facebook', 'LinkedIn', 'X', 'YouTube', 'Pinterest']
  const [selectedPlatforms, setSelectedPlatforms] = useState([])

  useEffect(() => {
    if (account) {
      setSelectedPlatforms(account.connectedPlatforms || [])
    }
  }, [account, isOpen])
  ...
  const handleSave = () => {
    if (account) {
      onSave(account.id, selectedPlatforms)
    }
    onClose()
  }
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={`Manage Connected Platforms — ${account.name}`} size="md">
      <div className="space-y-5">
        <p className="text-xs text-slate-500 dark:text-slate-400">
          Select or disconnect social media channels integrated with this business account:
        </p>
